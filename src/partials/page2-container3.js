...
const divDays = document.querySelector('.fiveDays');
...
async function getWeatherForDate() {
  try {
    const response = await axios(
      'https://api.openweathermap.org/data/2.5/forecast?APPID=8defc985a5e2c764076c53bf90c6c44e&units=metric&lang=en&q=Giurgiu'
    );
...
divDays.addEventListener('click', ev => {
  const clickedBtn = ev.target;
  if (clickedBtn.nodeName !== 'BUTTON') return;

  const isDaysButton = clickedBtn.classList.contains('days');
...
nextBtn.addEventListener('click', () => {
  const maxScrollPosition =
    forecastList.scrollWidth - container.clientWidth + 40;
